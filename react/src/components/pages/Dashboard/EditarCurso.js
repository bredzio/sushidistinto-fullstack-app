...
const EditarCurso = ({estadoModal, setEstadoModal}) => {
    const [inputs, setInputs] = useState({titulo:'', categoria: 'cursos', precio:'', newItem:''});
    const [items, setItems] = useState([]);
    const {setCursos, URI, editable} = useContext(DashboardContext);

    const loadEditableData = () => {
        setItems(editable.content);
        setInputs({
            titulo:editable.title,
            categoria: editable.category,
            precio: editable.price,
        });
    }
    ...
    const handleSubmit = async (event) => {
        ...
        await updateCurso(nuevoCursoObj);
    ...
    const updateCurso = async (newCourse) => {
        const res = await axios.put((URI+'/'+editable.id), newCourse);
        if (!res?.data) {
            alert("Ups! Ocurrió un error, intente nuevamente más tarde.");
        } else {
            alert(`Curso "${newCourse.title}" modificado satisfactoriamente!`)
            getCursos();
        }
    }
  ...
  return (
    <form onSubmit={handleSubmit} className='d-flex flex-column'>
        ...
        <MyLabel>Nuevo item:
        <MyflexContainer>
            <input type="text" name='newItem' className="form-control mb-2 mr-2" placeholder="Ej: Preparación de sashimis" onChange={handleChange}/>
        ...
        <MyflexContainer>
         ...
         <input type="submit" className='btn btn-primary' value={'Actualizar curso'} disabled={isBtnDisabled()}/>
  </form>
  )
}
